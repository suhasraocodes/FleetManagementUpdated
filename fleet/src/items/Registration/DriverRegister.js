...
function DriverRegister() {
  ...
  const onSubmit = async (data) => {
    ...
    // Append driver image if selected
    if (data.driverImage && data.driverImage.length > 0) {
      formData.append('driverImage', data.driverImage[0]);
    }

    try {
      await axios.post('http://localhost:8000/drivers/reg', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
      reset(); // Reset the form after successful submission
      toast.success('Driver registered successfully!'); // Use toast for success
    } catch (error) {
      console.error('Error registering driver:', error.response?.data || error.message);
      toast.error('Failed to register driver. Check if you are already registered'); // Use toast for error
    }
  };
